...
const SPENDING_DATA = [
  { month: 'Dec', amount: 2.10 },
  { month: 'Jan', amount: 3.80 },
  { month: 'Feb', amount: 5.20 },
  { month: 'Mar', amount: 4.90 },
  { month: 'Apr', amount: 7.30 },
  { month: 'May', amount: 6.40 },
];

const MONTHLY_BUDGET = 20;
const CURRENT_SPEND = 6.40;
const ANALYSES_RUN = 12;
const ANALYSES_THIS_MONTH = 5;
...
const CustomBarLabel = ({ x, y, width, value }) => (
  <text x={x + width / 2} y={y - 6} fill="#374151" fontSize={12} textAnchor="middle">
    ${value.toFixed(2)}
  </text>
);
...
const ProfilePage = () => {
  ...
  const budgetRemaining = MONTHLY_BUDGET - CURRENT_SPEND;
  const budgetPct = Math.round((CURRENT_SPEND / MONTHLY_BUDGET) * 100);
  const currentMonth = SPENDING_DATA[SPENDING_DATA.length - 1].month;
    ...
    <div style={{ background: '#f3f4f6', minHeight: '100vh', padding: '32px 24px' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '22px', fontWeight: 800, color: '#111', lineHeight: 1.2 }}>
              {user?.name || 'User'}
            </div>
        ...
        <div style={{ display: 'flex', gap: '16px', marginBottom: '20px' }}>
          <StatCard
            label="Analyses Run"
            value={ANALYSES_RUN}
            sub={`+${ANALYSES_THIS_MONTH} this month`}
          />
          <StatCard
            label="Spent This Month"
            value={`$${CURRENT_SPEND.toFixed(2)}`}
            sub={`of $${MONTHLY_BUDGET.toFixed(2)} budget`}
            valueColor="#2563eb"
            leftBorder
          />
          <StatCard
            ...
            sub={`Across ${sectorCount} sector${sectorCount !== 1 ? 's' : ''}`}
          />
          <StatCard
        ...
        <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
          ...
          <div style={{
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={SPENDING_DATA} barSize={42} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
                <XAxis
                  dataKey="month"
                  axisLine={false}
                  tickLine={false}
                  tick={{ fontSize: 13, fill: '#6b7280' }}
                  tickFormatter={(val) => val === currentMonth ? `${val} •` : val}
                />
                <YAxis
                  axisLine={false}
                  tickLine={false}
                ...
                <Bar dataKey="amount" radius={[4, 4, 0, 0]} label={<CustomBarLabel />}>
                  {SPENDING_DATA.map((entry) => (
                    <Cell
                      key={entry.month}
                      fill={entry.month === currentMonth ? '#2563eb' : '#d1d5db'}
                    />
                  ))}
                </Bar>
              </BarChart>
            ...
            <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px solid #f0f0f0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                ...
                <span style={{ fontSize: '14px', color: '#374151', fontWeight: 500 }}>
                  ${CURRENT_SPEND.toFixed(2)} of ${MONTHLY_BUDGET.toFixed(2)} used · {budgetPct}%
                </span>
              </div>
              ...
              <div style={{ fontSize: '13px', color: '#6b7280', marginTop: '8px' }}>
                ${budgetRemaining.toFixed(2)} remaining · Resets Jun 1, 2025 · 23 days
              </div>
            </div>
          </div>
          ...
          <div style={{ width: '300px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                ...
                <span style={{ fontSize: '13px', fontWeight: 600, color: '#111' }}>${CURRENT_SPEND.toFixed(2)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                ...
                <span style={{ fontSize: '13px', fontWeight: 600, color: '#111' }}>${MONTHLY_BUDGET.toFixed(2)}</span>
              </div>
              ...
              <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '8px' }}>
                {budgetPct}% used · resets in 23 days
              </div>
            </div>
...
const StatCard = ({ label, value, sub, valueColor, leftBorder }) => (
  <div style={{
    ...
    borderRadius: '12px',
    border: '1px solid #e5e7eb',
    borderLeft: leftBorder ? '4px solid #2563eb' : undefined,
  }}>
    <div style={{
